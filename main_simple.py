...
def parse_args():
    ...
    # output directory
    parser.add_argument('--resume', default='trained_models/model_79.pth', type=str, metavar='PATH',
...
def main(args):
    ...
    ssm_model, dit_model, _ = create_model(config, is_train=True)
    ...
    variance = VarianceSchedule(num_steps=config["num_sampling_steps"],
                                mode=config["diffusion_schedule_mode"])
    variance.to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_dict = train_one_epoch(
            ...
            diffusion=diffusion_util, DiT_model=dit_model,
