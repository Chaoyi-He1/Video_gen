...
from train_eval.train_text import *
from diffusers.models import AutoencoderKL
from diffusers import AutoencoderKLCogVideoX
from model.TextDecoder import TextDecoder
...
def main(args):
    ...
    # create model
    ssm_model, _, diffusion = create_model(config, is_train=True)
    tokenizer = CLIPTextTokenizer(model_dir=config["textencoder"])
    text_decoder = TextDecoder(config)
    ...
    text_decoder_without_ddp = text_decoder.module
    ...
    for p in ssm_model.module.parameters():
        ...
        if p.requires_grad:
            ...
            n_p_o += p.numel()
            layers += 1
    for p in text_decoder.module.parameters():
        n_p += p.numel()
        if p.requires_grad:
            p_to_optimize.append(p)
            n_p_o += p.numel()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        loss_dict = train_one_epoch(
            ...
            data_loader=dataloader, decoder=text_decoder,
            optimizer=optimizer, device=device, epoch=epoch,
            max_norm=args.clip_max_norm, scaler=scaler,
        ...
        if args.rank in [-1, 0]:
            if (epoch % args.save_freq == 0) or (epoch == args.epochs - 1):
                utils.save_on_master({
                    ...
                    'text_decoder': text_decoder_without_ddp.state_dict(),
